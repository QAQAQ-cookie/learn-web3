...
- 硬分叉，这意味着需要节点升级软件才能继续跟踪区块链，由于新旧节点不兼容，会造成区块链分叉，从而产生第二条链。
- 软分叉，是指比特币协议发生部分变化，这种情况下新旧节点仍可以接受新区块，保持网络一致。
...
- 区块时间戳，最先被记录的（时间戳较小）的交易为有效，其他会被拒绝。
- 6次确认，通常认为，当前区块之后又有5个区块被计算出来并连接到区块链上，在6个区块被确认之后，交易基本上无法篡改，被认为是完成了最终交易。
- 分叉及最长链原则，最长的链条会被确认为主链，其他分支链条上的节点将转换阵营，开始在较长的链条上工作，其他分叉出来的链将被网络抛弃。
